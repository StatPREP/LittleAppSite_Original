...
library(LittleApp2)
library(DT)
linear_app <- list(name = "hello", url = "goodbye")
```
...
It's natural to display a linear regression as a graph modeling the response variable as a function of the explanatory variables. In the Regression Modelling [`r Regression_app$name`](`r Regression$_appurl`) Little App, that function is shown as a line laid on top of the data. 
...
1. Open up the [`r Regression_app$name`](`r Regression$-appurl`) Little App. (See footnote^[<`r Regression_app$url`>]). Select `Little Apps` as the Source package,`NHANES2` as the data set and systolic blood pressure as the response variable. 
