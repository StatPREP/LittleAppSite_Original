...
Open up the [Center and Spread](`r Spread_app$url`) Little App (see footnote^[<`r Spread_app$url`>]). Make sure you click on Data in the top row. Set the Source package to Little Apps, the Data set to `Natality_2014`. Set the sample size to n = 200.

1. Set the response variable to be `dbwt`, the weight of the baby at birth. You'll see that there is variability, that is, not every baby weighs exactly the same.

2. In the App Controls button (looks like three parralel lines) show mean and the standard deviation by clicking on the boxes next to their name. Look closely at the annotation for the standard deviation. The standard deviation is a kind of measuring unit, like the inch marks on a ruler. The annotation is being drawn as a ruler. The mean is exactly in the middle and there are tick marks at ± 1 standard deviation and at ± 2 standard deviations.^[Sometimes the ± 2 marks don't fit within the frame, so those aren't included on the ruler.]
...
4. The numerical value of the standard deviation is the length between tick marks, just as an inch is the length between marks on a ruler. 
    ...
    * Use the measuring stick to find the numerical value for the standard deviation for the data shown in your plot. You do this by clicking on the +sd and draging your curser to the mean. 
    
    * Click on the "Stats" tab in the top bar and you will find printed the standard deviation as calculated directly from the data. Compare that number to the number you found in (a). They should be just about the same.
    * Use the measuring stick to find the length of the whole ruler being displayed. Compare that to the number you read in (b) from the Stats tab. 
    
    `r question_blank("*Describing how long the ruler is by how many standard deviations it is long.*", breaks = 1)`
    
5. In the App Control button, turn on the display of the 95% summary interval. See where the ends of the 95% summary interval fall along the standard deviation ruler.
...
6. The simple relationship between the 95% summary interval and the standard deviation often holds, but not always. Some variables consistently have the standard deviation ruler mis-aligned with the 95% summary interval.
    ...
    * Set the response variable to be APGAR10 score. Try several New Samples by clicking on the new sample button (looks like a pair of dice). 
    
    `r question_blank("*Does the 95% summary interval align with the ruler?*", breaks = 1)`
    
  
    * Back in the App Control button turn on the violin density display. 
    
    `r question_blank("*Is the density shape symmetric (top to bottom)? Does it have a very long tail?*", breaks = 2)`
    
    * Go back to `dbwt` and look at the violin. 
    
    `r question_blank("*How does the shape of this violin differ from that for APGAR10. Is it symmetric? Does it have a very long tail?*", breaks = 3)`
