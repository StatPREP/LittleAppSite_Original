...
date: "2020-06-16"
description: "StatPREP Class Activity"
weight: 5
...
library(knitr)
library(kableExtra)
center_app <- list(name = "hello", url = "goodbye")
```
...
Open up the [Center and Spread](`r Spread_app$url`) Little App (see footnote^[<`r Spread_app$url`>]). Make sure you click on Data in the top row. Set the Source package to Little Apps, the Data set to `Natality_2014`. Pick the sample size to 200 by clicking on n=50 and then picking n=200.

1. Set the response variable to be `mager`, the age of the mother at the time of birth. You'll see that there is variability, that is, not every mother is exactly the same age.
...
2. Perhaps the most natural way to measure the amount of variation is the length of the interval between the smallest value and the largest value. 
    ...
    * Use the measuring stick built in to the app to find the length of the interval from smallest to largest age. Just click on the largest age and then drag to the lowest age.
    
`r  SDSdata:::question_blank(c("*How long is it?*", "*What are the units?*"), breaks = 1)`


    
3. Press the New Sample button (looks like a pair of dice) and find the length of the interval from smallest to largest age again. For the new sample, are the maximum and minimum values the same as in the original sample? 
...
5. You might have noticed that only two of the points in each sample determine the max-to-min interval. For this reason, there is a lot of variation in the interval length from sample to sample. To avoid this, statisticians have invented another kind of interval to describe variation called the "95% summary interval." The idea is to avoid making the interval depend on just two points and instead let several points contribute. The 95% interval is designed to cover *almost all* of the data points, where "almost all" means 95%.
    ...
    * Turn on the "summary interval" display by clicking on the three paralel lines. This opens up a window to turn on some statistics and other commands. Click on the box that says Show Summary Interval. Keep the sample size  at n = 200. 
    ...
    * Press New Sample many times (the icon that looks like a pair of dice) to get a sense for whether the 95% interval or the max-to-min interval varies more from sample to sample.
...
You've already looked at how the max-to-min interval and the 95% summary interval  vary from one sample to another, holding the sample size constant. (We used n = 200.) Now you're going to see what happens as the sample size get's bigger and bigger. First, click on the snowflake icon. This will freeze the last sample you took that was size n=200. 
    ...
    b. Similarly, find the typical length of the 95% summary interval over many samples. 
    
        `r question_blank("*What's a typical range of the 95% interval?*", length = 5, breaks = 1)`
        
    c. Now increase the sample size to n = 1000. Press New Sample several times and keep track of how often the max-to-min interval is shorter than the one shown by the measuring stick for n = 200, and how often it is longer. You can compare the n-1000 graph to the n=200 graph that you froze by clicking on the Compare tab in the top row. 
    
        `r question_blank("What's a typical range of the max-to-min interval?", length = 5, breaks = 1)`
        
    d. Similarly compare the length of the 95% summary interval when n = 1000  to the length of the interval you recorded in step (b). 
