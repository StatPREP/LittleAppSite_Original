...
Open up the [Confidence and T](`r T_app$url`) Little App. (See footnote^[<`r T_app$url`>]).  Set the Source Package to `Little apps`, and the Data set to 'NHANSE2`. Set the response variable to `income_poverty` and the explanatory variable to `home_type`. Set the sample size to n = 100. Go into the App Control menu (the parallel lines icon). Check the box in the controls to show the mean values of the two groups.
