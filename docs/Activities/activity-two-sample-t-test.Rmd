...
library(LittleApp)
library(DT)
t_test_app <- list(name = "hello", url = "goodbye")
```
...
Open up the [Confidence and T](`r T_app$url`) Little App. (See footnote^[<`r T_app$url`>]). Set the Source package to `Little apps`, the Data set to `NHANES2`, set the response variable to `income_poverty`, and the explanatory variable to `home_type`. Set the sample size to n = 100 (click on the n=50 icon and choose n=100). Go into the Apps control (incon that looks like 3 parallel lines) Check the box in the control to show the mean values of the two groups.

1. Check the box in the control to show the *confidence interval* on the mean and the *t-interval*.  The t-interval is  displayed as a black, sideways-H shape. The t-interval is always *centered* on one of the two means. The t-test is based on whether the other mean falls inside the t-interval or outside.

    * Is the other mean inside or outside the t-interval?
 
2. The t-statistic is simply telling how far apart the two means are  with respect to the t-interval. Click on Stats in the upper tool bar to see the report from the t-test and read off the t-statistic. When the other mean is right at the boundary of the T-interval, the t statistic is 2. If the mean is farther outside the T interval, the t statistic is proportionately greater than 2. 
...
4. Check the "Shuffle groups" box. This simulates a situation where there is no systematic relationship between the response and explanatory variables. 
     ...
     * Note whether the t-interval includes both means or, much the same thing, whether the confidence intervals do not overlap. Press New Sample (pair of dice icon) several times to see whether the confidence-interval overlap (or lack of overlap) is consistent from one sample to another.
